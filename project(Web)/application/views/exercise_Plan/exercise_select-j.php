...
if ('exercise_Plan/exercise_select') {
    ...
    <div class="list-group" style="width: 15%;float: left;text-align: center;">
        <span class="fa fa-plus-square list-group-item active" style="font-size: 20px;width: 100%" aria-hidden="true"><a href="#" style="font-family: bokutachi;">
                運動選択</a></span>
        <span class="fa fa-calendar list-group-item" style="font-size: 20px;width: 100%" aria-hidden="true"><a href="#" style="font-family: bokutachi;"> 日付選択</a></span>
        <span class="fa fa-check-square-o list-group-item" style="font-size: 20px;width: 100%" aria-hidden="true"><a href="#" style="font-family: bokutachi;"> 計画完了</a></span>
    </div>
<?php } ?>
<div class="container-fluid">
        ...
        <div class="select_Part col-md-3">
            <div class="row" style="margin: auto">
                <div class="btn-group" role="group">
                    <button class="btn btn-primary raised" id="partICON" data-toggle="modal" data-target="#myModal"
                            style="font-family: bokutachi;font-size: 30px;color:white;">部位
                    </button>
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                    ...
                                    <h4 class="modal-title" id="myModalLabel" style="font-family: bokutachi;">部位選択</h4>
                                </div>
                                <div class="modal-body">
                                    ...
                                    <img src="/public/img/exercise/musle_icon.png" height="auto" alt="사람 이모티콘"
                                         usemap="#imgMap"/>
                                    <map id="imgMap">
                                        <a shape="rect" coords="159,82,217,138" alt="左の肩" title="左の肩"
                                           onclick="part_Click('Thigh')"></a>
                                        <a shape="rect" coords="354,76,298,121" alt="右の肩" title="右の肩"
                                           onclick="part_Click('Thigh')"></a>
                                        <input type="hidden" value="肩" id="Thigh">
                                        <a shape="rect" coords="187,221,254,357" alt="左太もも" title="左太もも"
                                           onclick="part_Click('Shoulder')"></a>
                                        <a shape="rect" coords="322,241,257,356" alt="右太もも" title="右太もも"
                                           onclick="part_Click('Shoulder')"></a>
                                        <input type="hidden" value="太もも" id="Shoulder">
                                    </map>
                                </div>
                                <div class="modal-footer">
                                    <h4 class="modal_select_text" style="font-family: bokutachi;">選択部位</h4>

                                    <h3 class="selectPartText" style="font-family: bokutachi;font-size: 30px;text-align: center; color:#1e87c3 "></h3>
                    ...
                    <button class="btn btn-primary raised" style="font-family: bokutachi;font-size: 30px;background-color:lightblue; color:#007299 ">
                        目的
                    </button>
                </div>
            </div>
            ...
            <div class="row" style="margin: auto">
                <div class="tt panel panel-success">
                    <div class="panel-heading" style="font-family: bokutachi;background-color: #1e87c3;color: white;"><h3>難易度</h3></div>
                    <div class="panel-body">
                        <div class="row" style="margin: auto">
                            <div class="col-md-4" id="e_part">
                                <a id="high" class="part_style outline" name="routine_level"
                                   style="font-family: bokutachi;font-size: 30px;border: 2px solid gray; color: gray;">上</a>
                                <input type="hidden" class="exer">
                            </div>
                            <div class="col-md-4" id="e_part">
                                <a id="middle" class="part_style outline" name="routine_level"
                                   style="font-family: bokutachi;font-size: 30px;margin-right: 10%;border: 2px solid gray; color: gray;">中</a>
                                <input type="hidden" class="exer">
                            </div>
                            <div class="col-md-4" id="e_part">
                                <a id="low" class="part_style outline" name="routine_level"
                                   style="font-family: bokutachi;font-size: 30px;border: 2px solid gray; color: gray;">下</a>
                                <input type="hidden" class="exer">
                            </div>
                        </div>
                    </div>
            ...
            <div class="row" style="margin: auto">
                <div class="tt panel panel-success">
                    <div class="panel-heading" style="background-color: #1e87c3;color: white;"><h3 style="font-family: bokutachi;">ルーチン選択</h3></div>
                    <div class="panel-body">
            ...
            <div class="row" style="margin: auto">
                <div class="btn-group" role="group" style="text-align: center">
                    <form class="exercise_info_form" action='/main_j/exercise_date' method='post'>
                        <input type=hidden id=qwe name='qwe'>
                        <input class="btn btn-primary btn-lg" type=submit value='完了' style="font-family: bokutachi;font-size: 30px">
                    </form>
                </div>
            </div>
        </div>
...
<script>
    ...
    $('#high').click(function () {
        ...
        $.ajax({
            type: "POST",
            url: "/main_j/exercise_Select_Part",
            data: {
                level: level,
                part: part
                ...
                for (var i = 0; i < routine_info.length; i++) {
                    ...
                    $('<a id="routine_nameH" class="part_style" onclick="aClickRoutine(this.parentNode.id)"style="font-size: 30px"></a>').attr("routine_index", routine_info[i].routine_list_index).html(routine_info[i].routine_name).appendTo(form);
                    $('#test1').append(form);
                }
    ...
    $(document).on('click', '#routine_nameH', function () {
        ...
        $.ajax({
            type: "POST",
            url: "/main_j/exercise_Select_routine",
            data: {
                ...
                for (var i = 0; i < routine_exercise_info.length; i++) {
                    ...
                    var form = $('<form style="font-family: bokutachi;float: left"></form>').addClass("exercise_form" + i);
                    $('<h1 class="exercise_info_text"></h1>').text(routine_exercise_info[i].exercise_name).appendTo(form);
                    $('<br>').appendTo(form);
                    $('<br>').appendTo(form);
                    $('<br>').appendTo(form);
                    $('<p class="exercise_info_text" style="display: inline;font-size: 22px"></p>').text(routine_exercise_info[i].exercise_info).appendTo(form);
                    $('<br>').appendTo(form);
                    $('<br>').appendTo(form);
                    $('<p class="text-center" style="font-size: 18px"></p>').text("セット数 :" + routine_exercise_info[i].number_of_set + "          セット当たりの回数 :" + routine_exercise_info[i].number_of_count).appendTo(form);
                    /* $('<p class="info_text_css"></p>').text("세트당 횟수 :" + routine_exercise_info[i].number_of_count).appendTo(form);*/
                    $('<button type="button" class="btn btn-success round center-block" style="font-size: 25px" onclick="movie_CTL(this.id)"></button>').attr('id', 'exercise_movie' + i).text('運動の動画').appendTo(form);
        ...
        $.ajax({
                ...
                data: {
                    ...
                    for (var i = 0; i < routine_exercise_info.length; i++) {
                        ...
                        $('<br>').appendTo(form);
                        $('<br>').appendTo(form);
                        $('<p class="text-center"></p>').text("セット数 :" + routine_exercise_info[i].number_of_set + "     セット当たりの回数 :" + routine_exercise_info[i].number_of_count).appendTo(form);
                        /* $('<p class="info_text_css"></p>').text("세트당 횟수 :" + routine_exercise_info[i].number_of_count).appendTo(form);*/
                        $('<button type="button" class="btn btn-success round center-block" onclick="movie_CTL(this.id)"></button>').attr('id', 'exercise_movie' + i).text('運動の動画').appendTo(form);
                        $('<input type="hidden"/>').attr("id", "exercise_movie_hidden" + i).val(routine_exercise_info[i].movie_path).appendTo(form);
                        $('<br>').appendTo(form);
                        $('<br>').appendTo(form);
                        $('.routine_form0').after(form);
